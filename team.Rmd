...
for (username in team_dirs) {
  index_file <- file.path("team", username, "_index.Rmd")
...
group_order <- c("Faculty", "Research Assistants", "OCS Alumni")
ordered_names <- c(
...
for (group_name in names(groups)) {
  cat(sprintf('\n<h2 class="group-header">%s</h2>\n', group_name))
  cat('<div class="team-grid">\n')
  
  for (username in names(groups[[group_name]])) {
    member <- groups[[group_name]][[username]]
    cat(sprintf('
<a href="team/%s/_index.html" class="team-card-link">
<div class="team-card">
<img src="%s" alt="%s" class="team-avatar">
<h3 class="team-name">%s</h3>
<div class="team-role">%s</div>
</div>
</a>
', username, member$avatar, member$name, member$name, member$role %||% ""))
  }
